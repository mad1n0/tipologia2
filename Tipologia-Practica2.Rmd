...
Para este caso recordamos el teorema del límite central que en el caso de poblaciones suficientemente grandes podemos asumir normalidad (2) (3). Sin embargo con el fin de realizar dicho estudio, evaluaremos la normalidad de diferentes muestras de nuestra población. Aunque lo ideal quizá hubiera sido hacer un estudio de la normalidad asintótica local o general (4) (5).

Para dicho estudio lo que vamos a hacer es aplicar los tests de Anderson-Darling (6) y Shapiro-Wilk en varias muestras de la población total para cada una de las variables. Solo aquellas variables que cuya mediana de todos los p-value de los tests den por encima de 0.05 serán tomados como normales según dichos tests.
...
<p>
  <a class="btn btn-primary" data-toggle="collapse" href="#boxcox" role="button" aria-expanded="false" aria-controls="boxcox">
    Gráficas muestra de las variables normales tras transformación Box-Cox.
  </a>
</p>
<div class="collapse" id="boxcox">
  <div class="card card-body">
...
<p>
  <a class="btn btn-primary" data-toggle="collapse" href="#homocedasticidad" role="button" aria-expanded="false" aria-controls="homocedasticidad">
    Gráficas residuo-valor para la confirmación visual de homocedasticidad
  </a>
</p>
<div class="collapse" id="homocedasticidad">
  <div class="card card-body">
...
### Correlación y regresión para las variables buenas

Usando las features que hemos mostrado para las que se cumple normalidad y homocedasticidad frente al precio, antes haremos un estudio sobre sus correlaciones, capacidad de regresión y responderemos algunas preguntas usando el método de las hipótesis.
...
good_features_data<-data.frame(c(vector(),1:nrow(aux_data)))

for (feature in unique(good_features)){
  good_features_data[,feature]<-aux_data[,feature]
}
good_features_data<-good_features_data[-1]
````
...
````{r}
cor_good_features<-cor(good_features_data)
heatmap(cor_good_features,main="Correlaciones de las variables buenas",cexRow = 0.9,cexCol = 0.65)
````

Podemos ver la correlación entre las variables de precio y de generación total con las de generación individuales. Aunque es interesante ver la relación entre la energía eólica y otras energías renovables.


### Regresión

````{r}
train_good<-subset(good_features_data[1:(nrow(good_features_data)*0.7),],select = -c(price.day.ahead))
test_good<-subset(good_features_data[(nrow(good_features_data)*0.7):nrow(good_features_data),],select = -c(price.day.ahead))

lm_price_good_features<-lm(price.actual ~ . ,data=train_good)

good_price_pred<-predict(lm_price_good_features,test_good)

rmse_lm_good_model<-rmse(preds = good_price_pred,actuals = test_good$price.actual)
````

El modelo de regresión lineal utilizando únicamente los datos que cumplían las propiedades nos da un valor de RMSE de `r round(rmse_lm_good_model,2)`. De lo cual podemos intuir una variación sobre la predicción de un 20%. Teniendo en cuenta las pocas variables escogidas, teniendo tantas más, se trata de un resultado bueno. Sin embargo el coeficiente de correlación es `r summary(lm_price_good_features)$adj.r.square` lo que nos lleva a pensar que no exista una relación lineal entre todas las variables y el precio; aunque puede que sí que exista relación. Esta "otra relación" la investigaremos brevemente con el uso de árboles de decisiones para hacer un modelo de regresión.

### Feature importance

````{r}
tree_good_model<-rpart(price.actual ~ ., data=train_good,control=rpart.control(minsplit=2, minbucket = 1,cp=0.00001))
nodes <- as.numeric(rownames(tree_good_model$frame))
max_depth_tree_model<-max(rpart:::tree.depth(nodes))

brief_tree_good_model<-prune.rpart(tree_good_model,cp=0.025)

fancyRpartPlot(brief_tree_good_model,caption="Extracto de la totalidad del árbol")

sum_nodes_tree_good<-sum(tree_good_model$variable.importance)
feature_importance_tree_model_good_percentage<-(100*tree_good_model$variable.importance/sum_nodes_tree_good)
kable(feature_importance_tree_model_good_percentage)
````


# **Efecto de las energías renovables sobre el precio**
¿Podemos decir que las energías renovables o no renovables generan una electricidad más barata? Encarecimiento o abataramiento de la energía según un origen no renovable o renovable. Tras ver el análisis para todas las variables, nos enfocamos en una de las preguntas: ¿la energía renovable encarece o abarata el precio? Para ello seguiremos el siguiente procedimiento:

1. Comprobar que existe una dependencia y estimar cuán fuerte puede ser.
2. Recoger separadamente los precios de los días cuando más se ha producido energía renovable y cuando más se ha producido energía no renovable.
3. Pasar un test t-Student, con el fin de probar que estadísticamente se puede afirmar que la media de los precios en ambos tipos de días difiere significativamente.
4. Veremos la diferencia de los valores de precios para ambos tipos de días, revisando si se abarata o se encarece.

## **¿Existe dependencia entre la generación no renovable y renovable con el precio? Contraste por hipótesis chicuadrado**

````{r}
ddata <- raw_data
#summary(ddata$price.actual)
quant_3_price<-quantile(ddata[,"price.actual"])[4]
ddata$price.actual <- ifelse(ddata$price.actual>=quant_3_price, 1, 0)
````

Vamos a definir como alto precio aquél una desviación estándar por encima de la media, `r quant_3_price`. A partir de este punto la variable price.actual en la copia del dataframe ddata será 1 en caso de ser superior o igual a `r quant_3_price` y 0 en caso contrario. 
Esto se hace con la intención de aplicar test de chi quadrado y

````{r}
norenovables <- ddata$generation.biomass + ddata$generation.fossil.brown.coal.lignite + ddata$generation.fossil.gas + ddata$generation.fossil.hard.coal + ddata$generation.fossil.oil + ddata$generation.nuclear
renovables <- ddata$generation.hydro.pumped.storage.consumption + ddata$generation.hydro.run.of.river.and.poundage + ddata$generation.hydro.water.reservoir + ddata$generation.solar + ddata$generation.wind.onshore
````

````{r}
quant_3_ren<-as.numeric(quantile(renovables[!(is.na(renovables))])[4])
quant_3_non_ren<-as.numeric(quantile(norenovables[!(is.na(norenovables))])[4])

renovables <- ifelse(renovables > quant_3_ren, 1, 0)
norenovables <- ifelse(norenovables > quant_3_non_ren, 1, 0)

renovables<-renovables[!(is.na(renovables))]
norenovables<-norenovables[!(is.na(norenovables))]
precios_energia<-ddata[!(is.na(ddata$price.actual)),"price.actual"]

m_rev<-min(c(length(precios_energia),length(renovables)))


tabla1 <- table(precios_energia[1:m_rev], renovables)
tabla2 <- table(precios_energia[1:m_rev], norenovables)
print(chisq.test(tabla1, correct = FALSE))
print(chisq.test(tabla2, correct = FALSE))
````

El valor p es menor que el nivel de significancia, con lo cuál podemos concluir que hay relación entre la generación de energía, tanto no renovable como la renovable, y el precio.


## **¿Cómo de fuerte es esa relación precio-energía? Correlación**

Para medir la fuerza de la dependencia de la generación de renovable y la generación no renovable con el precio, vamos a usar la correlación como valor numérico comparable.


## **¿Es mayor el precio de la electricidad cuando hay más generación por renovables o cuando hay más generación por no renovables? Hipótesis de contraste de la media de dos grupos**

````{r}
wr <- which(renovables == 1)
wnr <- which(norenovables == 1)

preciorenovables <- raw_data[wr,"price.actual"]
precionorenovables <- raw_data[wnr,"price.actual"]

m_rev<-min(c(length(preciorenovables),length(preciorenovables)))

info <- t.test(preciorenovables[1:m_rev], precionorenovables[1:m_rev], alternative = "less")
print(info)
````

Podemos confirmar estadísticamente que el valor medio de los precios para los momentos en que se produce más energía renovable es menor que cuando la generación de energías no renovables es alta. De forma que se puede interpretar como prueba a que la generación de energía mediante fuentes renovables abarata los precios de la electricidad frente a aquella generada mediante combustibles no renovables. 


## **Análisis predictivo. ¿Cuál es el precio de la electricidad sabiendo la generación, su rango de producción horario y anual?**

Por otro lado vamos a intentar ver si podemos hacer un modelo de regresión con un arbol de decisión cuyo *complexity parameter* (7) sea muy bajo para que el acierto en las poblaciones sea mayor. Para hacer la evaluación del modelo partido los datos en un set para entrenamiento y otro para testeo. La idea, como diferencia del los apartados anteriores, es crear un modelo que, aunque le perdamos la interpretabilidad sea capaz de predecir el valor del precio junto a la información sobre la hora del día, la estación del año y la generación de energía. Como posible aplicación sería la detección de anomalías en el precio de la electricidad, e incluso el fraude o manipulación de dichos valores.

````{r}
x_train<-subset(aux_data[1:(nrow(aux_data)*0.7),],select = -c(price.day.ahead))
x_test<-subset(aux_data[(nrow(aux_data)*0.7):nrow(aux_data),],select = -c(price.day.ahead))
````
...
Aquí tenemos una parte del modelo, hemos hecho *pruning* porque el árbol original tiene (8) ramas con una profundidad máxima de `r max_depth_tree_model` y `r length(tree_model$splits)` particiones; con lo que queda claro el motivo por el que recortar el árbol a mostrar.
...
- Por un lado hemos visto que la generación de energía mediante combustible fósil tiene un gran impacto en el precio, aunque sorprendentemente la energía hidroeléctrica también tiene un gran impacto en el precio; más incluso que la nuclear, la estacionalidad, o la eólica.

- Hemos demostrado estadísticamente que la cantidad de generación de energía tiene dependencia en el precio.

- Hemos demostrado que el precio de la electricidad es menor cuanta más energía renovable se emplea, frente al uso de las energías provenientes de fuentes no renovables.



# **Referencias**
(1) https://www.kaggle.com/nicholasjhana/energy-consumption-generation-prices-and-weather
(2) https://stat.ethz.ch/pipermail/r-help/2007-April/129620.html 
(3) **Carles Rovira Escofet**. *Teorema del límite central*. UOC
(4) https://web.stanford.edu/class/stats300b/Notes/contiguity-and-asymptotics.pdf
(5) https://journal.r-project.org/archive/2009/RJ-2009-015/RJ-2009-015.pdf
(6) https://www.itl.nist.gov/div898/handbook/prc/section2/prc213.htm
(7) http://www.learnbymarketing.com/tutorials/rpart-decision-trees-in-r/ 
(8) https://stackoverflow.com/questions/40897081/how-can-i-get-the-depth-of-the-tree-in-rpart-model
