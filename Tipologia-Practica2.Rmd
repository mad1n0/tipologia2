...
````{r}
heatmap(cor(good_features_data))
````

### Regresión

````{r}
lm_price_good_features<-lm(price.actual ~ . ,data=good_features_data)
...
## ¿Podemos decir que las energías renovables o no renovables generan una electricidad más barata? Encarecimiento o abataramiento de la energía según un origen no renovable o renovable
Tras ver el análisis para todas las variables, nos enfocamos en una de las preguntas: ¿la energía renovable encarece o abarata el precio? Para ello seguiremos el siguiente procedimiento:
...
tabla1 <- table(ddata$price.actual, renovables)
tabla2 <- table(ddata$price.actual, norenovables)
...
````{r}
#wr <- which(renovables > quant_3_ren)
#wnr <- which(norenovables > quant_3_non_ren)

#preciorenovables <- raw_data$price.actual[wr]
#precionorenovables <- raw_data$price.actual[wnr]

#info <- t.test(preciorenovables, precionorenovables, alternative = "less")
#print(info)
````
