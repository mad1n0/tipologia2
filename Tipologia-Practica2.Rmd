...
Estas son las variables que, según los tests de Anderson-Darling y Shapiro-Wilk, podemos decir que cumplen con normalidad estadísticamente.

````{r}

for (feature in unique(normal_features_list)){
  hist(aux_data[,feature],100,xlab="")
}
````
...
### Comprobación de homocedasticidad para la variable dependiente
...
### Comprobación de homocedasticidad de todas las variables explicativas


````{r}

for (name_i in names(aux_data)){
  for (name_j in names(aux_data)){
    if (name_j!=name_i){
      p_value_fligner<-fligner.test(aux_data[,name_i],aux_data[,name_j])$p.value
...
### Comprobación de la normalidad de todas las variables
Anderson-Darling (N+2)
...
for (i in names(aux_data)){
  p_value_adtest<-shapiro.test(aux_data[1:3000,i])$p.value
  if (p_value_adtest>0.05){
    print(p_value_adtest)  
  }
}
...
for (i in names(aux_data)){
  seed.sample(42)
  dsample <- sample(aux_data[1:30000,i],55, replace = TRUE)
  p_value_adtest<-shapiro.test(dsample)$p.value
  plot(density(dsample))
  if (p_value_adtest>0.05){
    print(p_value_adtest)  
  }
}
...
Sin tratado BoxCox, se puede descartar la hipótesis nula desde unas 60 muestras. Los plots no presentan una distribución normal.


Después de explorar un ratete, he visto que el test de shapiro sólo acepta 5k entradas de input. si se realiza el test con aux_data, que tiene 35k entradas, probablemente esté cogiendo los valores empezando por el 0 y no los de en medio. He probado a correr ambos tests con menos muestras, cogiéndolas aleatorias, y sí que pasa el test en algunos casos, deberíamos ver cómo hacer la selección de muestras.
...
Si hacemos un procesado BoxCox, 

## pruebas por contraste de hipótesis paramétricas
